...
<head>
	...
	<title>Insert title here</title>
	   ...
	   <link rel="stylesheet" href="<c:url value="/resources/css/custDash.css"></c:url>">
 ...
 <% List<ServiceRequest> sr=null;
   ...
   <div style="min-height: 100vh; padding-bottom: 170px;"> 
                    ...
                    <div class="offcanvas offcanvas-end navbar_offcanvas sidebarMain" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
                        <div class="offcanvas-body justify-content-end sidebarInner">
                            <div class="navcol_first navcol m-0 p-0">
                                <p class="nav-link">Welcome,<br>${user.getFirstName() } ${user.getLastName() }</p>
                            </div>
                    ...
                    <div class="d-flex align-items-center justify-content-center nav_right_icon position-relative">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end p-1 px-3">
                            <li class="border-bottom" style="font-size: 14px;">
                                ...
                                <p class="mb-1 fw-bold"><%=user.getFirstName()%> <p>
                            </li>
        ...
        <div class="position-relative custDash_header d-flex align-items-center justify-content-center">
            <p class="m-0">Welcome, <span class="text-bold"><%= user.getFirstName() %></span></p>
        </div>
                    ...
                    <table class="table table_1 p-3">
                        ...
                        <tbody>	
                            <c:forEach var="sr" items="<%=sr%>" varStatus = "i">
                            	...
                            	<tr>
	                                <td scope="row">${sr. getServiceRequestId()}</td>
	                                <td class="service_date cursorPointer" href="#serviceDetails" data-bs-toggle="modal" id="${sr. getServiceRequestId()}" onclick="myFunction($(this).attr('id'))">
	                                    <a class="d-flex text-decoration-none align-items-center">
	                                        ...
	                                        <p class="mb-0 ms-1">
	                                        	<fmt:formatDate type = "date" value = "${sr.getServiceStartDate()}" pattern="dd/MM/yyyy"/>
	                                        </p>
	                                    </a>
	                                  <a class="d-flex text-decoration-none">
	                                        <p class="m-0">
	                                        	<fmt:formatDate type = "time" value = "${sr.getServiceStartDate()}" pattern="hh:mm"/>-<span id="endTime${i.index }"></span>
	                                        </p>
	                                    </a> 
	                                </td>
	                                <c:if test="${sr.getServiceId() != sr.getServiceProviderId() }">
		                                <td>
		                                    <div class="d-flex">
		                                        <div class="sp_icon me-2 rounded-circle d-flex align-items-center justify-content-center">
		                                            <%-- <c:forEach var="u" items="${users }">
		                                        		<c:if test="${u.user_id == sr.getServiceProviderId()}"> 
		                                        ...
		                                        <div class="d-flex flex-column">
		                                            <p class="mb-0">
		                                        			...
		                                        			<c:if test="${user.getUserId() == sr.getServiceProviderId()}"> ${user.getFirstName()} ${user.getLastName() }</c:if>
		                                            ...
		                                            <div class="d-flex align-items-center">
		                   								...
		                   								<%-- <c:forEach items="${spRating}" var="spRating">
													        ...
													        <c:if test="${spRating.key == sr.getServiceProviderId()}">
	                                ...
	                                <c:if test="${sr.getUserId() == sr.getServiceProviderId()}">
	                                	<td></td>
	                                </c:if>
	                                <td>
	                                    <p class="m-0 text-center  table_payment fw-bold">$ ${sr.getTotalCost() }</p>
	                                </td> 
	                                <td class="button_main text-center">
	                                    <a href="#scheduleServiceRequest" data-bs-toggle="modal" role="button" class="reschedule_button rounded-pill text-light text-decoration-none" data-spID ="${sr.getServiceRequestId()}" onclick="$('#rescheduleServiceReqId').val($(this).attr('data-spID'))">Reschedule</a>
	                                    <a href="#cancelServiceRequest" data-bs-toggle="modal" role="button" class="cancel_button rounded-pill text-light text-decoration-none" data-spID ="${sr. getServiceRequestId()}" onclick="$('#cancelServiceReqId').val($(this).attr('data-spID'))">Cancel</a>
	                                </td>
	                            </tr>
                ...
                <div class="modal-dialog modal-dialog-centered modal-sm">
                    <div class="modal-content">
                        ...
                        <div class="modal-body ">
                            ...
                            <form action="service_cancel" class="d-flex flex-column" id="cancelReqForm" method="post">
